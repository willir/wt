...
public:
  ...
  void setShowQueriesHandler(const std::function<void(const std::string&)> &handler);
  void showQueries(const std::string& sqlAsQuery) const;
...
protected:
  ...
  TransactionIsolationLevel transactionIsolationLevel_;
...
private:
  ...
  std::function<void(const std::string&)> queryLogger_;
};
