...
class PostgresException : public NonRetriableException
{
public:
  PostgresException(const std::string& msg)
    : NonRetriableException(msg)
  { }

  PostgresException(const std::string& msg, const std::string& code)
    : NonRetriableException(msg, code)
  { }
};
...
void Postgres::exec(const std::string& sql, bool showQuery)
{
  ...
  std::unique_ptr<PGconn, AllResultsReader> on_err_res_clearer(conn_);
  for (;;) {
    ...
    std::unique_ptr<PGresult, ResultClearer> result_clearer(result);
    handleErr(err, result, conn_);
  }
  on_err_res_clearer.release();
}
...
void Postgres::startTransaction()
{
  ...
  case TransactionIsolationLevel::RepeatableRead:
    exec("start transaction isolation level repeatable read", false);
    break;
  case TransactionIsolationLevel::Serializable:
    exec("start transaction isolation level serializable", false);
    break;
  }
}
