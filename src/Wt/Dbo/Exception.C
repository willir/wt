...
#ifdef WT_WITH_UNWIND
  : std::runtime_error("Code:" + code + ". " + error + " at\n" + backtrace()),
#else
...
StaleObjectException::StaleObjectException(const std::string& id, 
					   const char *table, 
             int version)
  : RetriableException(std::string("Stale object, ") + table + ", id = " + id +
...
ObjectNotFoundException::ObjectNotFoundException(const char *table,
						 const std::string& id)
  : NonRetriableException(std::string("Object not found in ") + table + ", id = " + id)
{ }
